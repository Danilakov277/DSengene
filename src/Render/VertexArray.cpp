...
namespace Renderer {
	...
	void VertexArray::addBufer(const VertexBuffer& vertexBuffer, const VertexBufferLayout& layout)
	{
		bind();
		vertexBuffer.bind();
		...
		for (unsigned int i = 0; i<layoutElements.size();++i)
		{
			...
			glVertexAttribPointer(currentAttribIndex, currentLayoutElement.count, currentLayoutElement.tupe, currentLayoutElement.normalized, layout.getStride(), offset);

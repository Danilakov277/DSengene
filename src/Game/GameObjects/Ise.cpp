#include "Ise.h"
#include"../../Resources/ResourcesManger.h"
#include "../../Render/Sprite.h"


Ise::Ise(const glm::vec2& position, const glm::vec2& size, const float rotation, const float layer) :
	IGameObject(IGameObject::EObjectType::Ice,position,size,rotation,layer),
	m_sprites(ResourceManger::getSprite("ise_block")),
	m_blockOffsets{ glm::vec2(0,m_size.y / 2.f),
										  glm::vec2(m_size.x / 2.f,m_size.y / 2.f),
										  glm::vec2(0,0),
										  glm::vec2(m_size.x / 2.f,0) }
{
	

}

void Ise::renderBlock(const EBlockLocation eBlockLocation) const
{


		m_sprites->render(m_position+m_blockOffsets[static_cast<size_t>(eBlockLocation)], m_size/2.f, m_rotation,m_layer);
	
}


void Ise::render() const
{
	renderBlock(EBlockLocation::BottomLeft);
	renderBlock(EBlockLocation::BottomRight);
	renderBlock(EBlockLocation::TopLeft);
	renderBlock(EBlockLocation::TopRight);
}

